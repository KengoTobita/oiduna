...
- SessionCompiler で ScheduleEntry に変換される

**例**:
```python
# キックドラム（ステップ0）
kick = PatternEvent(step=0, offset=0.0, params={"sound": "bd", "gain": 0.8})

# スウィングハイハット（ステップ64、オフセット0.5）
hihat = PatternEvent(step=64, offset=0.5, params={"sound": "hh", "gain": 0.6})
```
...
- **step vs offset**: ScheduleEntryは両方を持つ
  - `step: int` (0-255) - 量子化されたステップ番号（インデックス用）
  - `offset: float` (0.0-0.999...) - ステップ内相対位置（スウィング、トリプレット用）
