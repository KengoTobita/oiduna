...
class StateProducer(Protocol):
    ...
    Produces (sends) state updates from oiduna.infrastructure.execution to oiduna_api.
...
class StateConsumer(Protocol):
    ...
    Consumes (receives) state updates in oiduna_api from oiduna.infrastructure.execution.
